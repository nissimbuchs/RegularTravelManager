...
**Note:** Database schema uses snake_case column names (e.g., `employee_id`, `manager_id`) per SQL conventions, while API request/response bodies use camelCase (e.g., `employeeId`, `managerId`) per the architecture's field naming conventions.
