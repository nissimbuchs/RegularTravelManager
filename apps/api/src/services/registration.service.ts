...
export class RegistrationService {
  ...
  async markEmailVerified(email: string, token: string): Promise<boolean> {
    try {
      ...
      const tokenResult = await db.query(tokenQuery, [email, token]);
      ...
      await db.query(employeeQuery, [email]);
  ...
  async checkEmailExists(email: string): Promise<{ cognito: boolean; database: boolean }> {
    ...
    const dbResult = await db.query(dbQuery, [email]);
  ...
  async createEmployeeRecord(
    ...
    const employeeId = await this.generateEmployeeId('EMP');
  ...
  async getRegistrationStatus(email: string): Promise<{
    ...
    `;
    const verificationResult = await db.query(verificationQuery, [email]);
   ...
   */
  private async generateEmployeeId(prefix: 'EMP' | 'MGR' | 'ADM'): Promise<string> {
    try {
