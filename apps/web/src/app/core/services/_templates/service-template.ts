...
export class TemplateService {
  ...
  getItem(id: string): Observable<ExampleData> {
    return this.http.get<{data: ExampleData}>(`${this.baseUrl}/${id}`).pipe(
  ...
  getItems(): Observable<ExampleData[]> {
    return this.http.get<{data: ExampleData[]}>(`${this.baseUrl}`).pipe(
      map(response => response.data)
    );
  }
  ...
  getItemsWithPagination(): Observable<{items: ExampleData[], pagination: any}> {
    return this.http.get<{data: {items: ExampleData[], pagination: any}}>(`${this.baseUrl}/paginated`).pipe(
      map(response => response.data)
    );
  }
  ...
  createItem(item: CreateRequest): Observable<ExampleData> {
    return this.http.post<{data: ExampleData}>(`${this.baseUrl}`, item).pipe(
      map(response => response.data)
    );
  }
  ...
  updateItem(id: string, item: Partial<ExampleData>): Observable<ExampleData> {
    return this.http.put<{data: ExampleData}>(`${this.baseUrl}/${id}`, item).pipe(
      map(response => response.data)
    );
  }
